...
| 단계 | 파일 | 역할 |
|------|------|------|
| 1 | data_loader.py | 554 JSON 파싱, 장르 정규화(66→30), 파생 수치 5종, pkl 캐시 |
| 2 | embedding.py | 499D 하이브리드 벡터 (그룹별 L2 정규화 + 가중치) + StandardScaler |
...
| data/ | 캐시 (kmdb_processed.pkl, text_embeddings.npy, saved_weights.json) |
| results/ | HTML 시각화 7종 + 종합 보고서 (summary.html) |
...
시뮬레이티드 어닐링: T₀=1.0, α=0.92, step=0.2, patience=10, max_iter=50.
이웃 생성 → 복합 점수 평가 → 수락/거절(exp(ΔS/T)) → 온도 냉각.
신뢰도 = 0.4·stability + 0.4·reproducibility + 0.2·convergence.
정확도 = 4조건 통과율 (sim≥0.5, genre≥0.3, text≥0.3, 0.15≤div≤0.6).
저장 조건: 신뢰도 ≥ 50% AND 정확도 ≥ 50%.

## 가중치 영속화 (config.py)

저장: `data/saved_weights.json` (weights + metadata).
로드: `get_effective_weights()` — 저장 파일 우선, 없으면 config 기본값.
대시보드: 시작 시 자동 로드, "저장 및 반영"으로 저장, "초기화"로 파일 삭제.
...
python main.py --sensitivity         # 81조합 민감도 분석 + 기여도 순위
...
- tensorflow 설치 환경에서는 protobuf 충돌 → tensorflow 제거 필요
- saved_weights.json 삭제 시 config.py 기본 가중치로 복원
...
- **가중치 변경**: config.py WEIGHT_* 또는 대시보드 슬라이더 또는 시뮬레이션 최적화
...
- **최적화 목적함수**: optimizer.py _evaluate_weights() 내 계수 조정 (0.35/0.30/0.25/0.10)
